...
def move_n_files(source_path, des_path):
    ...
    for i in range(800,900):
        copy2(str(f"{audio_ind[i]}"), str(d_path / f"grid_s1_val" / f"audio")) 
        copy2(str(f"{video_ind[i]}"), str(d_path / f"grid_s1_val" / f"video")) 
    for i in range(900,1000):
        copy2(str(f"{audio_ind[i]}"), str(d_path / f"grid_s1_test" / f"audio")) 
        copy2(str(f"{video_ind[i]}"), str(d_path / f"grid_s1_test" / f"video")) 
